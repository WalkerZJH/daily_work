#!/usr/bin/env python
"""Rebuild the alive prediction model-selection story notebook."""
...
if trainability is not None:
    ...
    md(cells, "## 3. 多模型 baseline 与模型家族比较")
    code(
        cells,
...
display(Markdown(guardrails))''',
    ...
    md(
        cells,
        "## 4.1 2024 数据采集覆盖扩张解释\n\n"
        "现有时间漂移不应只解释为终端行为漂移。2024 年 recurring entity count 快速增长，同时 label positive rate 在年初下降、下半年回升，"
        "更符合 data regime shift / coverage expansion：线上采购要求变严后，系统采集到更多医院采购订单，监控样本池发生扩张。\n\n"
        "因此不建议直接删除旧数据；更合理的做法是保留 pre-2024 作为旧采集制度样本，将 2024 标注为 transition regime，"
        "后续等 2025/2026 标签窗口闭合并物化后，再做 post-expansion calibration / validation。",
    )
    code(
        cells,
...
display(xgb_family.head(30) if xgb_family is not None else pd.DataFrame([{"warning": "model family comparison missing"}]))''',
    ...
    md(cells, "2022-only 是 smoke test；expanded train 缓解样本不足，但时间漂移仍存在。XGBoost 作为 nonlinear challenger 保留，但没有稳定超过 Logistic。")
    md(cells, "## 6. Feature ablation 与概率候选收敛")
    code(
        cells,
...
plot_model_probability_metrics(prob_candidates, title="Probability Candidate Consolidation")''',
    ...
    md(cells, "`all_features` 不一定最好；`base_recency_frequency_only` 是早期低泄露基线；`value_features` 不进入主概率模型；Logistic 在概率指标上更稳。")
    md(cells, "## 7. Calibration v1 / rolling-origin / stabilization")
    code(
        cells,
...
"""))''',
    ...
    md(cells, "v1 阶段 raw / Platt / isotonic 改善不一致；rolling-origin 显示 Logistic 稳定但不能直接 promote，主要问题是时间漂移与校准不稳定。")
    md(cells, "## 8. Feature stability v1 与 calibration v2")
    code(
        cells,
...
pd.DataFrame([
    ...
    md(cells, "normalized_recency_v1 与 frequency_decay_v1 降低漂移；calibration v2 中 raw 最优，最终 promote `logistic_regression + frequency_decay_v1 + raw`。")
    md(cells, "## 9. Demand-shape routing 与标签口径审查")
    code(
        cells,
...
def main() -> None:
    ...
    output = root / "notebooks/02_alive_prediction_model_selection_story.ipynb"
