...
class ArtifactRiskScorer(BaseRiskScorer):
    ...
    def score(self, feature_frame: pd.DataFrame) -> pd.DataFrame:
        ...
        return _score_frame(feature_frame, probs, "artifact", self.artifact.manifest.artifact_id)
...
class RuleBaselineScorer(BaseRiskScorer):
    ...
    def score(self, feature_frame: pd.DataFrame) -> pd.DataFrame:
        ...
        return _score_frame(feature_frame, probs, "dry_run_rule_baseline", self.artifact_id)
...
def _score_frame(features: pd.DataFrame, probs: Any, source: str, artifact_id: str) -> pd.DataFrame:
    out = features[
        [
            "entity_id",
            "manufacturer_code",
