from __future__ import annotations

import pandas as pd

from .base import DetectorOutput, RuntimeDetector, to_frame


class FrequencyDropDetector(RuntimeDetector):
    detector_name = "purchase_frequency_fluctuation_warning"
    detector_family = "frequency"

    def run(self, candidates: pd.DataFrame, features: pd.DataFrame) -> pd.DataFrame:
        merged = candidates[["candidate_id", "entity_id", "horizon"]].merge(
            features[["entity_id", "horizon", "frequency_ratio"]],
            on=["entity_id", "horizon"],
            how="left",
        )
        rows = []
        for row in merged.itertuples():
            value = float(getattr(row, "frequency_ratio", 1) or 0)
            hit = value < 0.5
            rows.append(
                DetectorOutput(
                    candidate_id=str(row.candidate_id),
                    detector_name=self.detector_name,
                    detector_family=self.detector_family,
                    hit_flag=bool(hit),
                    severity="medium" if hit else "low",
                    confidence="medium",
                    evidence_type="frequency_drop",
                    reason_code="recent_frequency_below_baseline" if hit else "frequency_context",
                    metric_name="frequency_ratio",
                    metric_value=value,
                    visibility_level="business_visible",
                    caveat="frequency evidence needs business review",
                    forbidden_claims="causal_claim",
                )
            )
        return to_frame(rows)
