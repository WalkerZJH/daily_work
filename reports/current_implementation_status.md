...
## 2026-07-15 P2-01 One-shot 事实型工作台

`oneshot.html` 已收口为新进终端事实工作台。正式链路仅使用当前结果批次中的
`oneshot_terminals.parquet`，支持生产企业范围、正式事实排序和服务端分页；缺表时返回
`ONESHOT_RESULT_NOT_AVAILABLE`，不会静默回退到 Recurring `risk_entities`。

客户页面仅展示医院、药品、生产企业、首购日期、首次采购时点金额、距首购天数、
数据月份、截止日期和批次信息。复购倾向、预计复购金额、预测优先级和模型排序依据已从
One-shot 客户链路移除。P2-01 未新增物化、One-shot 详情或预测模型，Detector 按日物化仍是
不阻塞本模块的独立任务。
...
代码主链路已实现并通过定向测试；旧 `risk_entity`、`is_high_risk`、`high_risk_detector_evidence` 是历史技术命名，不代表候选对象经过固定高风险阈值准入。正式 `2025-12/full-recurring-v3` 已物化 88,515 行 Recurring 主干候选，并通过 manifest 双计数、`validate_result_batch` 与模型层校验；未训练模型、未覆盖旧批次。`formal-v2-raw` 的 1,229 行只保留为按观察日期读取的独立 Detector 历史批次，不作为月度候选池。
...
| 新 full-recurring 批次 | implemented | `full-recurring-v3` 有正式 `manifest.json`，`full_recurring_count == persisted_recurring_count == 88,515`，并通过结果契约与模型层校验；API 月度候选发现链路读取该批次 |
