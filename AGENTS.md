...
Daily Detector is a fact-only service, not a monthly-result-batch stage. Its production entry supports an explicit repeatable `--detector-id`: `python -m production_pipeline.run_daily_detector --raw-batch-dir <raw_batch> --observation-date YYYY-MM-DD --run-id <id> --detector-id <detector_id>`. It atomically publishes an immutable `detector_run_date=YYYY-MM-DD/detector_id=<detector_id>/batch_id=...` component. Updating one Detector must publish only that Detector for the affected dates; never rerun every Detector or monthly prediction by default. It must read only the raw purchase facts required by its rules and must not call monthly feature engineering, monthly scoring, candidate selection, or write into `report_month=*/batch_id=*`. The authoritative runbook is `risk_model_core/DETECTOR_PRODUCTION_BOUNDARY.md`.
