...
需求 detector 固定按四类业务 category 展示：`price_warning`、`delivery_response`、`terminal_change`、`sales_fluctuation`。`common_preprocess` 只用于预处理分类，不作为 health 页面业务 detector 展示。

所有需求 detector 都必须输出统一 `DetectorRunResult / DetectorEvidence` 字段：`detector_id`、`category`、`family`、`hit`、`severity`、`confidence`、`reason_code`、`statistics`、`evidence_items`、`related_entities`、`sample_order_ids`、`warnings`、`narrative`。前端应优先展示后端返回的 `narrative`，不要自行拼接核心业务解释。

阈值统一从 `DetectorRuntimeConfig` 读取。`auto_baseline` 只用于算法验证和测试，不能等同客户配置阈值；`ml_first`、`dl_first` 当前仅预留接口，未实现时必须返回 warning 并 fallback。
