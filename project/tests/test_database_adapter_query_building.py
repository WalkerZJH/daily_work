...
def test_sql_adapter_builds_projected_columns_and_filters_without_select_star() -> None:
    ...
    assert spec.filters["采购时间_from"] == date(2026, 5, 20)
    assert spec.filters["采购时间_to"] == date(2026, 6, 1)
    assert spec.filters["企业编码"] == "ENT-1"
    assert spec.filters["省"] == "江苏省"
    assert spec.row_limit == 5000


def test_sql_adapter_defaults_to_small_window_when_dates_missing() -> None:
    adapter = SQLTableSourceAdapter(as_of_date=date(2026, 6, 24))

    spec = adapter.build_query_spec(available_columns={"采购时间", "药品编码"})

    assert spec.filters["采购时间_from"] == date(2026, 6, 10)
    assert spec.filters["采购时间_to"] == date(2026, 6, 24)
    assert spec.row_limit == 5000
