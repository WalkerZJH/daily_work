...
class DetectorResultService:
    ...
    def clues(
        ...
        *,
        ...
        clues = _sort_frame(clues, ["display_rank", "created_at"], ascending=True)
        total = int(len(clues))
        current_page = max(int(page), 1)
        current_size = min(max(int(page_size), 1), 200)
        start = (current_page - 1) * current_size
        items = [_clue_item(row) for _, row in clues.iloc[start : start + current_size].iterrows()]
        return {
            "ready": bool(items),
            "source": SOURCE,
            "items": items,
...
def _clue_item(row: pd.Series) -> dict[str, Any]:
    return {
        ...
        "is_monthly_high_risk_entity": _bool(row.get("is_monthly_high_risk_entity")),
